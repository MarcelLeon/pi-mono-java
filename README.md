...
- [x] Add OAuth device-code polling helper and a minimal GitHub Copilot device-flow client that delays the first token poll, honors server-provided `slow_down` intervals, exchanges the GitHub OAuth token for a Copilot API token, and persists that token plus refresh material to a local `api_key` credential store
- [x] Expose a credential-backed GitHub Copilot provider that is unavailable without a persisted access token, selectable by model id once credentials exist, refreshes expired Copilot API tokens from persisted GitHub refresh material, discovers account-specific models from the Copilot `/models` endpoint, sends tested non-streaming OpenAI-compatible chat requests through a Copilot token-derived API base URL, merges `ProviderHeaderContributor` headers with request headers, parses returned `tool_calls` into Java tool-call metadata, and serializes assistant `tool_calls` history for continuation requests
...
- [x] Honor request-scoped OpenAI provider headers for non-streaming HTTP requests while keeping authorization managed by the provider
- [x] Merge Java `ProviderHeaderContributor` headers before OpenAI/Anthropic/Bedrock/GitHub Copilot non-streaming provider calls, with explicit request headers taking precedence and provider-managed auth/signing headers protected where each transport owns them
...
- [x] Send OpenAI non-streaming tool schemas, parse `tool_calls` into assistant message metadata, and serialize assistant `tool_calls` history for continuation requests
...
- [x] Honor request-scoped Anthropic provider headers for non-streaming HTTP requests while keeping `x-api-key`/authorization managed by the provider
- [x] Retry Anthropic non-streaming provider errors whose response body explicitly asks callers to retry
...
- [x] Merge Java `ProviderHeaderContributor` headers into Bedrock non-streaming requests without overriding SigV4-managed headers
- [x] Retry Bedrock non-streaming provider errors whose response body explicitly asks callers to retry
...
- [x] Apply `edit` replacements as literal text and tolerate model-supplied extra replacement metadata when required fields are valid
...
- `mvn -pl pi-tools -am test -Dtest=EditFileToolTest,BashToolTest,ReadFileToolTest -Dsurefire.failIfNoSpecifiedTests=false -Djava.version=17`: pass (literal `edit` replacements, extra replacement metadata tolerance, bash timeout validation, and BMP-to-PNG `read` tests).
...
- `mvn -pl pi-llm -am test -Dtest=GitHubCopilotOAuthClientTest,OAuthDeviceCodePollerTest -Dsurefire.failIfNoSpecifiedTests=false -Djava.version=17`: pass (GitHub Copilot OAuth device-flow startup, delayed first token poll, RFC +5s `slow_down`, server-provided `slow_down` interval handling, Copilot API token exchange, and local `api_key` credential-store persistence through a deterministic fake transport).
- `mvn -pl pi-llm -am test -Dtest=GitHubCopilotLLMProviderTest -Dsurefire.failIfNoSpecifiedTests=false -Djava.version=17`: pass (credential-backed Copilot provider availability, model-id selection, account-specific model discovery through a deterministic `/models` response, expired Copilot API token refresh from persisted GitHub refresh material, Copilot token-derived API base URL selection, dynamic Copilot headers, provider-header contributors, non-streaming chat response parsing, non-streaming `tool_calls` metadata parsing, and assistant `tool_calls` continuation serialization through a deterministic fake transport).
...
- `mvn -pl pi-llm -am test -Dtest=OpenAIClientTest,OpenAIConfigTest,OpenAILLMProviderTest -Dsurefire.failIfNoSpecifiedTests=false -Djava.version=17`: pass (OpenAI HTTP error body, retryable provider errors, HTTP 524 timeout retry classification, DS4 context-overflow classification, request output-token floor clamping, default model, request-scoped API key/env/model/options/provider headers, provider header contributors, Azure Foundry endpoint, response content parsing, usage/reasoning token metadata, incomplete output-length response, OpenAI image attachment content parts, non-streaming tool schema request, `tool_calls` metadata parsing, assistant `tool_calls` continuation serialization, and `(no tool output)` placeholder for empty tool result payloads).
- `mvn -pl pi-llm -am test -Dtest=AnthropicClientTest,AnthropicConfigTest,AnthropicLLMProviderTest -Dsurefire.failIfNoSpecifiedTests=false -Djava.version=17`: pass (Anthropic-compatible Claude Sonnet 5 catalog, request-scoped API key/env auth/provider headers, provider header contributors, Messages API request headers/body, Claude 5 thinking payload, image attachment content blocks, non-streaming tool schema request without internal execution metadata leakage, `tool_use` metadata parsing, provider-native `tool_result` content blocks including empty-result placeholders, HTTP error body, retry-instruction error handling, response content, thinking content, and usage metadata tests).
- `mvn -pl pi-llm -am test -Dtest=BedrockClientTest,BedrockConfigTest,BedrockLLMProviderTest -Dsurefire.failIfNoSpecifiedTests=false -Djava.version=17`: pass (Bedrock Claude Sonnet 5 catalog, SigV4 signing from explicit, shared-profile, environment, request-scoped AWS credentials, or request-scoped AWS profile credentials, provider header contributors without overriding provider-managed headers, non-streaming invoke payload, Claude 5 thinking payload, prompt-cache block, image attachment content blocks, non-streaming tool schema request without internal execution metadata leakage, `tool_use` metadata parsing, provider-native `tool_result` content blocks including empty-result placeholders, HTTP error body, retry-instruction error handling, response content, thinking content, usage, and region metadata tests).
