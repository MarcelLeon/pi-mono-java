...
| Skills and prompt templates | `packages/coding-agent` resource loader | `pi-cli` | Partial | Java discovers user-global prompts/skills, trusted project-local `.pi/prompts/*.md`, `.pi/skills/*/SKILL.md`, `.agents/skills/*/SKILL.md`, and installed package prompt/skill files under `.pi/packages/*/prompts` and `.pi/packages/*/skills`; it applies trusted `.pi/settings.json` `prompts`/`skills` override patterns as a project-local config delta over local or inherited global resources; it expands simple `{{key}}` prompt templates and injects basic `/skill:name` instructions into CLI conversations, but does not execute extension-provided tools or the full Agent Skills lifecycle. |
...
| Extensions, themes, packages | `packages/coding-agent` resource/package loader | `pi-cli` file discovery only | Gap | Upstream can load executable extensions, themes, and Pi packages; Java only discovers local and installed-package prompt/skill files and does not install packages or execute package/extension code. |
...
- Pass request-scoped provider headers and Java `ProviderHeaderContributor` headers through OpenAI, Anthropic, Bedrock, and GitHub Copilot non-streaming HTTP requests while keeping auth and SigV4 headers managed by provider credential resolution or transport boundaries.
...
- Register a GitHub Copilot provider that remains unavailable until a persisted access token exists, participates in model-id provider selection, refreshes expired Copilot API tokens from persisted GitHub refresh material, discovers account-specific models from the Copilot `/models` endpoint, derives the Copilot API base URL from the token `proxy-ep`, adds dynamic Copilot request headers, merges `ProviderHeaderContributor` headers with explicit request-header precedence, parses tested non-streaming OpenAI-compatible chat responses, exposes returned Copilot `tool_calls` as `metadata.toolCalls`, and serializes assistant `tool_calls` history for continuation requests.
...
- Retry OpenAI, Anthropic, and Bedrock non-streaming provider errors whose response body explicitly asks callers to retry.
- Convert CLI image attachments into OpenAI non-streaming `image_url` content parts.
- Send OpenAI non-streaming tool schemas, expose returned `tool_calls` as `metadata.toolCalls`, and serialize assistant `tool_calls` history for Java-side continuation requests.
...
- Discover local and installed-package prompt templates and skill manifests in Pi-compatible directories.
...
- Full Agent Skills execution lifecycle, extension-provided tools, themes, Pi package installation/update flows, and upstream's interactive `pi config` TUI selector.
...
- Full GitHub Copilot streaming transport parity and real-account auth/tool-call validation beyond the tested minimal OAuth device-flow client plus credential-backed non-streaming provider transport; full automatic compaction integration beyond serialized split-turn summary orchestration.
...
- Full upstream extension-runtime event delivery and RPC/orchestrator process integration beyond minimal `get_entries`/`get_tree`; Java has a provider-header contributor boundary for OpenAI, Anthropic, Bedrock, and GitHub Copilot non-streaming calls, but not full extension-runtime `before_provider_headers` dispatch.
