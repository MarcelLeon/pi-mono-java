...
- Pass request-scoped provider headers and Java `ProviderHeaderContributor` headers through OpenAI and Anthropic non-streaming HTTP requests while keeping auth headers managed by provider credential resolution.
...
- Full upstream extension-runtime event delivery and RPC/orchestrator process integration beyond minimal `get_entries`/`get_tree`; Java has a provider-header contributor boundary, but not full extension-runtime `before_provider_headers` dispatch.
