...
- Reuse an OAuth device-code polling helper that can delay the first token poll, apply RFC 8628 `slow_down` +5s fallback, and prefer a server-provided `slow_down` interval when present.
...
- Full GitHub Copilot OAuth/login integration beyond the tested generic OAuth device-code polling interval helper, plus full automatic compaction integration beyond serialized split-turn summary orchestration.
...
mvn -pl pi-llm -am test -Dtest=OAuthDeviceCodePollerTest
...
mvn -pl pi-llm -am test -Dtest=OAuthDeviceCodePollerTest -Dsurefire.failIfNoSpecifiedTests=false -Djava.version=17
