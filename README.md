...
- [x] Add OAuth device-code polling helper and a minimal GitHub Copilot device-flow client that delay the first token poll, honor server-provided `slow_down` intervals, and persist successful access tokens to a local `api_key` credential store
- [x] Expose a credential-backed GitHub Copilot provider skeleton that is unavailable without a persisted access token and selectable by model id once credentials exist
...
- `mvn -pl pi-llm -am test -Dtest=GitHubCopilotOAuthClientTest,OAuthDeviceCodePollerTest -Dsurefire.failIfNoSpecifiedTests=false -Djava.version=17`: pass (GitHub Copilot OAuth device-flow startup, delayed first token poll, RFC +5s `slow_down`, server-provided `slow_down` interval handling, and local `api_key` credential-store persistence through a deterministic fake transport).
- `mvn -pl pi-llm -am test -Dtest=GitHubCopilotLLMProviderTest -Dsurefire.failIfNoSpecifiedTests=false -Djava.version=17`: pass (credential-backed Copilot provider availability, model-id selection, and explicit unimplemented chat transport failure).
