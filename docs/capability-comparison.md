...
- Pass request-scoped provider headers through OpenAI and Anthropic non-streaming HTTP requests while keeping auth headers managed by provider credential resolution.
- Reuse an OAuth device-code polling helper and minimal GitHub Copilot device-flow client that can delay the first token poll, apply RFC 8628 `slow_down` +5s fallback, and prefer a server-provided `slow_down` interval when present.
