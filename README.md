...
- [x] Expose a credential-backed GitHub Copilot provider that is unavailable without a persisted access token, selectable by model id once credentials exist, and able to send a tested non-streaming OpenAI-compatible chat request through a Copilot token-derived API base URL
...
- `mvn -pl pi-llm -am test -Dtest=GitHubCopilotLLMProviderTest -Dsurefire.failIfNoSpecifiedTests=false -Djava.version=17`: pass (credential-backed Copilot provider availability, model-id selection, Copilot token-derived API base URL selection, dynamic Copilot headers, and non-streaming chat response parsing through a deterministic fake transport).
