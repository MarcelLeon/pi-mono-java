...
- Reuse an OAuth device-code polling helper and minimal GitHub Copilot device-flow client that can delay the first token poll, apply RFC 8628 `slow_down` +5s fallback, prefer a server-provided `slow_down` interval when present, and persist successful access tokens to a local `api_key` credential store.
- Register a GitHub Copilot provider skeleton that remains unavailable until a persisted access token exists, then participates in model-id provider selection while returning an explicit unsupported error for chat transport.
...
- Full GitHub Copilot chat transport, credential refresh, and real-account auth validation beyond the tested minimal OAuth device-flow client plus credential-backed provider availability; full automatic compaction integration beyond serialized split-turn summary orchestration.
